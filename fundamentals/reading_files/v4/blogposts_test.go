...
import (
	"errors"
	blogposts "github.com/LightBulbfromSpace/kld-go-learning/reading_files/v4"
	"io/fs"
	"reflect"
	"testing"
	"testing/fstest"
...
func assertPost(t *testing.T, got, want blogposts.Post) {
	t.Helper()
	if got.Title != want.Title || got.Description != want.Description || got.Body != want.Body {
		t.Errorf("1. got %+v but want %+v", got, want)
	}
	if !reflect.DeepEqual(got.Tags, want.Tags) {
		t.Errorf("2. got %+v, got == nil (%t), but want %+v, want == nil (%t)",
			got.Tags, got.Tags == nil, want.Tags, want.Tags == nil)
	}
}
